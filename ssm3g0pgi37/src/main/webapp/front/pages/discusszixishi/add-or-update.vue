			...
			async onSubmitTap() {
				...
				//跨表计算
						...
						if(this.ruleForm.id){
							...
							oet = await this.$api.add(`discusszixishi`, this.ruleForm);
						}
						uni.setStorageSync('discusszixishiCleanType',true);
						let rs2 = await this.$api.list('discusszixishi',{page: 1,limit: 10000,refid: this.ruleForm.refid})
						let totalscore = 0
						for(let x in rs2.data.list){
							totalscore += Number(rs2.data.list[x].score)
						}
						let rs3 = await this.$api.info('zixishi',this.ruleForm.refid)
						rs3.data.totalscore = Number((totalscore / rs2.data.list.length).toFixed(2))
						await this.$api.update('zixishi',rs3.data)
						if(this.cross){
